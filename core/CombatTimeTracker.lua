...
local function UpdateTimerFrame(self, elapsed)
    ...
    if currentDBKey and HRT.data.combatTimeTracker[currentDBKey] then
        local bestTime = HRT.data.combatTimeTracker[currentDBKey]
...
function InitializeFrames()
	combatTimeTrackerFrame = CreateFrame("Frame", "BossTimerFrame", UIParent)
	combatTimeTrackerFrame:SetSize(160, 85)
	...
	local bg = combatTimeTrackerFrame:CreateTexture(nil, "BACKGROUND")
	bg:SetAllPoints(combatTimeTrackerFrame,true)
	bg:SetColorTexture(0, 0, 0, 0.6)

	timerText = combatTimeTrackerFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightHuge")
	timerText:SetPoint("TOP", combatTimeTrackerFrame, "TOP", 0, -10)
	timerText:SetText("00:00.000")

	bestTimeBar = CreateFrame("StatusBar", nil, combatTimeTrackerFrame)
	...
	bossNameText = combatTimeTrackerFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
	bossNameText:SetPoint("TOP", bestTimeBar, "BOTTOM", 0, -5)
	bossNameText:SetText(L["tracker.wait-fight"])

	difficultyText = combatTimeTrackerFrame:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
	difficultyText:SetPoint("TOP", bossNameText, "BOTTOM", 0, -2)
	difficultyText:SetText("")

	local closeButton = CreateFrame("Button", nil, combatTimeTrackerFrame)
	closeButton:SetSize(20, 20)
	closeButton:SetPoint("TOPRIGHT", combatTimeTrackerFrame, "TOPRIGHT", -2, -2)
	...
	resetButton = CreateFrame("Button", nil, combatTimeTrackerFrame)
	resetButton:SetSize(16, 16)
	resetButton:SetPoint("BOTTOMRIGHT", combatTimeTrackerFrame, "BOTTOMRIGHT", -4, 4)
    ...
    combatTimeTrackerFrame:SetPoint(HoratumSettings.point, UIParent, HoratumSettings.relativePoint, HoratumSettings.xOfs, HoratumSettings.yOfs)
